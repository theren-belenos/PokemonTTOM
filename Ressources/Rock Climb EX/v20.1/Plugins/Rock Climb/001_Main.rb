...
def pbRockClimbEnd
  ...
  x = $game_player.x
  y = $game_player.y
  if $game_map.terrain_tag(x, y).rock_climb_tag && !$game_player.pbFacingTerrainTag.rock_climb_tag
    $game_temp.rock_climb_base_coords = [x, y]
...
class Game_Player < Game_Character
  ...
  def update_move
    if !@moved_last_frame || @stopped_last_frame   # Started a new step
      if !pbTerrainTag.ice && $PokemonGlobal&.rockclimbing
