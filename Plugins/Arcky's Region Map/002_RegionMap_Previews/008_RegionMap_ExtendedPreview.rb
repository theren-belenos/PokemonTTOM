class PokemonRegionMap_Scene
  ...
  def getExtendedInfo(check = false)
    ...
    gameMaps.each do |gameMap|
      ...
      percentage[:progress] = [seen, caught, defeated, found].each { |value| toNumber(value) }.sum
  ...
  def getTrainerInfo(map, district)
	...
	ARMSettings::CountItemsToMainMap.each do |main|
      if main[0] == map.id
        main[1].each do |id|
          totalTrainers += @globalCounter[:gameMaps][:trainers][id]
          trainer = $ArckyGlobal.trainerTracker&.dig(district, :maps, id) unless $ArckyGlobal.trainerTracker&.dig(district, :maps)&.empty?
          defeated += trainer[:defeated] unless trainer.nil?
        end
      end
    end
    if totalTrainers == 0
  ...
  def extendedSub
    loop do
      ...
      updateButtonInfo if !ARMSettings::ButtonBoxPosition.nil?
      updateSprites
      if Input.trigger?(Input::LEFT) || Input.trigger?(Input::BACK)
  ...
  def extendedEnc
    ...
    loop do
      ...
      if Input.trigger?(Input::BACK)
        ...
        disposeSprites
        getEncIcons
      elsif nil #Input.trigger?(ARMSettings::SelectSpeciesButton)
        pageInfo += 1
        if pageInfo > 1
          pageInfo = 0
        end
