class PokemonRegionMap_Scene
  ...
  def getAvailableActions(name = "", replaceName = "")
    ...
    @mapActions = {
      ...
      :ChangePage => { #(@previewBox.isExtShown && @getData && @getData.length > 1 && @extendedBox.isMain) ||
        condition: (@extendedBox.isSubOne && @tableData.length > 1) && !@extendedBox.isSubTwo,
        text: _INTL("Change Page"),
        button: [Input::UP, Input::DOWN],
        order: 5
      },
      #:ChangeSpecies => {
      ...
      :QuickFly => {
        condition: @mode == 1 && enableMode(ARMSettings::CanQuickFly) && !getFlyLocations.empty? && @region == 0,
      ...
      :Quit => {
        condition: @previewBox.isHidden && !@searchActive && @region == 0,
	  ...
	  :TownDev => {
        condition: @region == 1,
        text: _INTL("Reconstruction Project"),
        button: "",
        order: 90
      },
    }
