...
class VPM_DateAndTimeHud < Component   
  def initialize
    @last_time = pbGetTimeNow.strftime("%Hh%M")
  end
  ...
  def update
    super
    refresh if (pbGetTimeNow.min != @last_time) && !@menu.should_exit && !@sprites["overlay"].nil?
  end

  def refresh
    time  = pbGetTimeNow 
    day  = $town.getCurrentDay
	if day[0] == "Weekend"
		text = day[0] + " " + day[1].to_s
		text2 = time.strftime("%Hh%M")
	else
		text = day[0] + ", Week " + day[1].to_s
		text2 = (day[2] == 0) ? "Morning" : "Evening"
	end	
    @sprites["overlay"].bitmap.clear
